...
class ScenarioReport(Logger):
    ...
    def writejsonreport(
        ...
        try:
            ...
            MAIN_LOGGER.error("Could not write JSON report '%s': %s" % (json_path, str(_err)))
    ...
    def readjsonreport(
        ...
        :return: Scenario data read from the JSON report file.
        ...
        try:
            ...
            MAIN_LOGGER.error("Could not read JSON report '%s': %s" % (json_path, str(_err)))
            self.debug("Exception", exc_info=sys.exc_info())
            return None
        finally:
            self.resetindentation()
    ...
    def _scenario2json(
        ...
        self.debug("Generating JSON report for scenario '%s'" % scenario_definition.name)
        ...
        self.popindentation()
        self.debug("JSON report generated for scenario '%s':" % scenario_definition.name)
        self.debuglongtext(json.dumps(_json_scenario, indent=2), max_lines=20)
    ...
    def _json2scenario(
        ...
        self.debug("Reading scenario from JSON")
        self.pushindentation()
        self.debug("JSON: %s..." % json.dumps(json_scenario)[:100])
        ...
        self.debug("Name: %s" % _scenario_definition.name)
        ...
        for _json_error in json_scenario["errors"]:  # type: JSONDict
            ...
            self.debug("Error: %s" % str(_scenario_definition.execution.errors[-1]))
        ...
        for _json_warning in json_scenario["warnings"]:  # type: JSONDict
            ...
            self.debug("Warning: %s" % str(_scenario_definition.execution.warnings[-1]))
        ...
        self.debug("Time statistics: %s" % str(_scenario_definition.execution.time))
    ...
    def _step2json(
        ...
        self.debug("Generating JSON report for %s" % str(step_definition))
        self.pushindentation()
        ...
        self.popindentation()
        self.debug("JSON report generated for %s:" % str(step_definition))
        self.debuglongtext(json.dumps(_json_step_definition, indent=2), max_lines=10)
    ...
    def _json2step(
        ...
        self.debug("Reading step instance from JSON")
        self.pushindentation()
        self.debug("JSON: %s..." % json.dumps(json_step_definition)[:100])
        ...
        self.debug("Description: %s" % repr(_step_definition.description))
        ...
        if ("executions" not in json_step_definition) or ("actions-results" not in json_step_definition):
            ...
            for _json_step_execution in json_step_definition["executions"]:  # type: JSONDict
                self.debug("Building step execution instance from JSON")
                self.pushindentation()
                self.debug("JSON: %s..." % json.dumps(_json_step_execution)[:100])
                ...
                for _json_warning in _json_step_execution["warnings"]:  # type: JSONDict
                    ...
                    self.debug("Warning: %s" % str(_step_execution.warnings[-1]))
    ...
    def _actionresult2json(
        ...
        self.debug("Generating JSON report for %s" % str(action_result_definition))
        self.pushindentation()
        ...
        for _action_result_execution in action_result_definition.executions:  # type: ActionResultExecution
            ...
            for _subscenario_execution in _action_result_execution.subscenarios:  # type: ScenarioExecution
                try:
                    self.debug("Generation JSON report sor sub-scenario '%s'" % _subscenario_execution.definition.name)
                    self.pushindentation("  | ")
        ...
        self.popindentation()
        self.debug("JSON report generated for %s:" % str(action_result_definition))
        self.debuglongtext(json.dumps(_json_action_result_definition, indent=2), max_lines=10)
    ...
    def _json2actionresult(
        ...
        self.debug("Reading action/result instance from JSON")
        self.pushindentation()
        self.debug("JSON: %s..." % json.dumps(json_action_result_definition)[:100])
        ...
        self.debug("Description: %s" % repr(_action_result_definition.description))

        for _json_action_result_execution in json_action_result_definition["executions"]:  # type: JSONDict
            self.debug("Reading action/result execution instance from JSON")
            self.pushindentation()
            self.debug("JSON: %s..." % json.dumps(_json_action_result_execution)[:100])
            ...
            self.debug("Time: %s" % str(_action_result_execution.time))

            _action_result_execution.evidence = _json_action_result_execution["evidence"].copy()
            self.debug("Evidence: %s" % repr(_action_result_execution.evidence))
            ...
            for _json_error in _json_action_result_execution["errors"]:  # type: JSONDict
                ...
                self.debug("Error: %s" % str(_action_result_execution.errors[-1]))
            ...
            for _json_warning in _json_action_result_execution["warnings"]:  # type: JSONDict
                ...
                self.debug("Warning: %s" % str(_action_result_execution.warnings[-1]))
