...
def importmodulefrompath(
    ...
    assert script_path.is_file(), "No such file '%s'" % script_path
    assert script_path.name.endswith(".py"), "Not a Python script '%s'" % script_path
    ...
    try:
        ...
        assert sys.version_info >= (3, 6), "Incorrect Python version %s, 3.6 required at least" % sys.version
        ...
        if _module is None:
            raise ImportError("Could not load '%s'" % script_path)
        ...
        REFLEX_LOGGER.debug("importmodulefrompath('%s') => %s" % (script_path, repr(_module)))
        return _module
    except Exception as _err:
        REFLEX_LOGGER.debug(str(_err), exc_info=sys.exc_info())
        raise _err
...
def getloadedmodulefrompath(
    ...
    for _module_name in sys.modules:  # type: str
        if hasattr(sys.modules[_module_name], "__file__"):
            if pathlib.Path(sys.modules[_module_name].__file__).samefile(script_path):
                _module = sys.modules[_module_name]
                break

    REFLEX_LOGGER.debug("getloadedmodulefrompath('%s') => %s" % (script_path, repr(_module)))
    return _module
...
def checkfuncqualname(
    ...
    def _walkmodule(
        ...
        try:
            REFLEX_LOGGER.debug("Walking %s" % repr(module))
            ...
            for _func_name, _func in inspect.getmembers(module, inspect.isfunction):  # type: str, types.FunctionType
                ...
                if _res is not None:
                    return _res
            if func_name == "<module>":
                REFLEX_LOGGER.debug("_walkmodule(): '%s' matches '%s'! => returning '%s'" % (repr(module), func_name, qualname(module)))
    ...
    def _walkclass(
        ...
        if cls.__module__ != qualname(_module):
            ...
            # REFLEX_LOGGER.debug("_walkclass(%s): '%s' != '%s'" % (repr(cls), cls.__module__, qualname(_module)))
            return None

        _res = None  # type: typing.Optional[str]
        try:
            REFLEX_LOGGER.debug("Walking class %s" % repr(cls))
            REFLEX_LOGGER.pushindentation()
            # Inner classes.
    ...
    def _walkfunction(
        ...
        if (not hasattr(func, "__module__")) or (func.__module__ != qualname(_module)):
            ...
            # REFLEX_LOGGER.debug("_walkfunction(%s): '%s' != '%s'" % (repr(func), func.__module__, qualname(_module)))
            return None
    ...
    def _walkcode(
        ...
        try:
            REFLEX_LOGGER.debug("Walking '%s' %s" % (code_name, repr(code)))
            REFLEX_LOGGER.pushindentation()
            ...
            for _const in code.co_consts:  # type: typing.Any
                if _last_code is not None:
                    if isinstance(_const, str):
                        ...
                        REFLEX_LOGGER.warning("%s following %s expected to be of type str, %s found" % (repr(_const), repr(_last_code), type(_const).__name__))
                    _last_code = None
    ...
    if not _fqn:
        REFLEX_LOGGER.warning("Could not find fully qualified name for %s:%d:%s()" % (file, line, func_name))
    REFLEX_LOGGER.popindentation()
...
def codelinecount(
    ...
    while _index < len(code.co_lnotab):
        ...
        if _index < len(code.co_lnotab):
            ...
            _index += 1
            REFLEX_LOGGER.debug("codelinecount(): byte-code-addr(%+d) = %d, lineno(%+d) = %d" % (
                _byte_code_incr, _byte_code_addr,
                _lineno_incr, _lineno,
            ))
    return _lineno
