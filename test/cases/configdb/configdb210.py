...
class ConfigDb210(scenario.test.TestCase):
    ...
    def __init__(self):  # type: (...) -> None
        ...
        scenario.test.TestCase.__init__(
            ...
            objective="Check that a JSON configuration file can be loaded with a given root configuration key,"
        ...
        self.addstep(LoadConfigFile(scenario.test.paths.datapath("conf.json"), root_key=self.tmp_root_key1))
        ...
        self.section("Save JSON file")
        self.addstep(SaveConfigFile(self.mktmppath(suffix=".json"), root_key=self.tmp_root_key1))

        self.section("Reload saved JSON file")
        ...
        self.addstep(CheckConfigValue(f"{self.tmp_root_key2}.x.y[0].z", read_as=None, expected_type=int, expected_value=100))
        self.addstep(CheckConfigValue(f"{self.tmp_root_key2}.x.y[1].z", read_as=None, expected_type=int, expected_value=101))
        self.addstep(CheckConfigValue(f"{self.tmp_root_key2}.x.y[2].z", read_as=None, expected_type=int, expected_value=102))
        self.addstep(CheckConfigValue(f"{self.tmp_root_key2}.x.y[3].z", read_as=None, expected_type=int, expected_value=103))
