...
SCENARIO_REQ_BASELINE = scenario.ReqBaseline("scenario")  # type: scenario.ReqBaseline


with SCENARIO_REQ_BASELINE:
    ...
    CAMPAIGNS = scenario.Req(
        ...
        text="""
            ...
            The log output, in the end, gives a summary of the scenario execution status, and highlights the errors and warnings
            (see MULTIPLE_SCENARIO_EXECUTION).
        """,
    )  # type: scenario.Req
...
def save(
    ...
    """
    Saves :mod:`scenario.reqs` features as the :attr:`._paths.REQ_DB` requirement file.
    ...
    # Update `REQ_DB` file.
    scenario.logging.info(f"Updating scenario requirement file '{_paths.REQ_DB}'")
    SCENARIO_REQ_BASELINE.req_db.dump(_paths.REQ_DB)
    ensurelicenseheader(_paths.REQ_DB)

    # Configure `REQ_DB` file as 'scenario.req_db_files'.
    if set_default_req_file:
        scenario.logging.info(f"Setting '{_paths.REQ_DB}' as the default requirement file")
