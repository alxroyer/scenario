...
def savesphinxverbosity(
    ...
    # Save Sphinx verbosity as our `scenario` *debug main* option.
    # The *debug main* option, defined by the `CommonLoggingArgs.debug_main()` property,
    # relies on the info stored within the `Args._args` opaque member.
    _debug_main = (verbosity > 0)  # type: bool
    scenario.Args.getinstance()._args.debug_main = _debug_main  # noqa  ## Access to protected member _args
