...
def simplifyreferences(
    ...
    _logger.debug("simplifyreferences(docname=%r, element=%r, short_ref=%r): element.attributes=%r",
                  docname, element, short_ref, element.attributes)
        ...
        if isinstance(_child, docutils.nodes.Text):
            ...
            if short_ref is not None:
                ...
                if _short_ref.endswith(_child):
                    _logger.debug("Text %r is even shorter than %r, don't change it", _child, short_ref)
