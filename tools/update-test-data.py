...
class UpdateTestData:
    ...
    def _updateknownissuesscenario(self):  # type: (...) -> None
        ...
        _test_data = TestData(scenario.test.paths.KNOWN_ISSUES_SCENARIO, [
            ...
            # Known issues.
            "#---", "#000", "#001", "#002", "#003", "#004", "#011", "#012", "#013", "#014",
            # Failures.
            "Step-fail", "step010-fail",
        ])  # type: TestData
        updatedataexpectations(_test_data)
    ...
    def _updatesyntaxerrorscenario(self):  # type: (...) -> None
        ...
        # Temporarily fix the syntax error in order to make the module loadable for reflection.
        _initial_content = scenario.test.paths.SYNTAX_ERROR_SCENARIO.read_bytes()  # type: bytes
        try:
            scenario.test.paths.SYNTAX_ERROR_SCENARIO.write_bytes(_initial_content.replace(
                b'syntax error  # location: syntax-error',
                b'"syntax error"  # location: syntax-error',
            ))
            ...
            # Restore the syntax error in the end.
            scenario.test.paths.SYNTAX_ERROR_SCENARIO.write_bytes(_initial_content)
